...
export function clean(id = '') {
    DEBUG && console.warn(Date.now(), storeKeyName, 'clean', id)
    if (localStorage.getItem(storeKeyName)) {
        DEBUG && console.warn(Date.now(), storeKeyName)
...
export function saveToken(token, expire, remember: boolean = false) {
    ...
    DEBUG && console.debug(Date.now() + '', storeKeyName, token, expire, remember)
...
function handleVisibilityChange() {
    if (doc[hidden]) {
        ...
        if (userSessionHasChanged) {
            ...
            localStorage.setItem('debug', 'session change reload')
...
window.addEventListener('storage', ev => {
    if (ev.key === storeKeyName) {
        DEBUG && console.info(Date.now(), 'storage change', storeKeyName)
